...
async function showApp() {
  ...
  try {
    ...
    document.getElementById('loginError').textContent = 'Login succeeded, but app data failed to load. Please refresh and try again.';
  }
}
...
async function refreshAll() {
  const statusFallback = {
    isLocked: false,
    isNinosOverrideActive: false,
    isScheduleOpen: true,
    scheduleOpenAtUtc: null
  };

  const [matchData, round32MatchData, predictionData, predictionStatusData, schedulePredictionData] = await Promise.all([
    fetchJsonSafe('/api/matches', []),
    fetchJsonSafe('/api/round32/matches', []),
    fetchJsonSafe(`/api/predictions/${user.id}`, {}),
    fetchJsonSafe(`/api/predictions/status?userId=${user.id}`, statusFallback),
    fetchJsonSafe('/api/predictions/schedule', [])
  ]);

  matches = Array.isArray(matchData) ? matchData : [];
  round32Matches = Array.isArray(round32MatchData) ? round32MatchData : [];
  predictions = predictionData || {};
  schedulePredictionMatches = Array.isArray(schedulePredictionData) ? schedulePredictionData : [];
